...
| 版本 | 日期 | 说明 |
|------|------|------|
| v1.0 | 2026-07-18 | 初始版本，iOS 浅色主题 |
| v1.1 | 2026-07-19 | **UI 重设计**：深色主题 + 渐变配色 + 布局精简 + SM-2 算法 + 难记词追踪 + 王者击杀音效 |
...
| **设置** | 发音开关、每日目标数、复习上限，数据导入/导出 | ✅ |
| **首页统计** | 学习进度、已掌握数、难点词数、连续天数 | ✅ |
...
| 圆角 | 20px/12px 两级圆角系统 |
| 动画 | fadeIn 0.35s + slideUp + pulse |
| 背景效果 | 径向渐变光晕（紫色/青色/粉色） |
| 毛玻璃 | `backdrop-filter: blur(24px)` 导航栏 |

**v1.0 → v1.1 前后对比：**

| 对比项 | v1.0 | v1.1 |
|--------|------|------|
| 主题色 | 蓝色 `#007AFF` | 紫色渐变 `#6C5CE7 → #00CEC9` |
| 背景 | 浅灰 `#F2F2F7` | 深色 `#0F0F1A` |
| 首页统计卡 | 5 个 | 4 个（去掉"今日待复习"） |
| 首页按钮 | 4 个 | 2 个（去掉"今日已学""复习难点词"） |
| 导航栏 | 纯白 + 蓝线 | 毛玻璃 + 紫色渐变顶线 |
| 学习按钮 | 4 个文字按钮 | 4 个图标+文字按钮 |
| 默写按钮 | 3 个（提示/确认/下一个） | 2 个（提示/确认） |
| 圆角 | 16px | 20px/12px 两级 |
| 阴影 | `rgba(0,0,0,0.08)` | `rgba(0,0,0,0.3)` + 发光效果 |
| 动画 | 只有卡片进场 | fadeIn + slideUp + pulse + shimmer |
| 滚动条 | 默认 | 自定义窄滚动条 |
| 分类标签 | 浅背景 | 深色卡片 + 渐变激活状态 |
...
```
5aec970  2026-07-19  v1.1 UI重设计：深色主题 + 渐变配色 + 布局精简
4a661e3  2026-07-19  UI重设计：深色主题 + 渐变配色 + 布局精简
d987fef  2026-07-19  添加王者击杀音效mp3 7个
a3ab6ca  2026-07-19  更新文档为SSH推送方式，清理旧部署脚本
...
| 标签 | 对应 commit | 说明 |
|------|------------|------|
| `v1.0` | `bb3629f` | 旧版浅色主题（回滚用） |
| `v1.1` | `5aec970` | 最新版深色主题 |

回滚命令：`git checkout v1.0`
